...
def get_dataset_class(name):
    if name == "amass":
        ...
        return KIT
    elif name == "humanfeedback":
        from data_loaders.humanml.data.dataset import HumanFeedback
...
def get_dataset(name, num_frames, split='train', hml_mode='train'):
    ...
    if name in ["humanml", "kit"]:
        ...
        dataset = DATA(max_frames=num_frames)
    else:
...
def get_dataset_loader(name, batch_size, num_frames, split='train', hml_mode='train'):
    ...
    loader = DataLoader(
        ...
        num_workers=1, drop_last=False, collate_fn=collate
    )
